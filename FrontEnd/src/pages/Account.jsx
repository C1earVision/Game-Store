            ...
            <table className="table-auto w-full text-left border-collapse border border-gray-700">
              ...
              <tbody>
                {orders.map((order, index) => (
                  <tr
                    key={index}
                    className={`transition-all ${order.Status === "DELIVERED"
...
const AdminProducts = () => {
  ...
  const handleSearch = async (value) => {
    setSearchTerm(value)
    const url = `https://game-store-zo3k.vercel.app/api/v1/products?Name=${value}`
    const response = await axios.get(url);
    setProducts(response.data.games);
  }
  ...
  const handleDelete = async (productId) => {
    try {
      const user = JSON.parse(localStorage.getItem("user"));
      await axios.delete(`https://game-store-zo3k.vercel.app/api/v1/user/admin/${productId}`, {
        headers: {
          Authorization: `Bearer ${user.token}`,
        },
      });
